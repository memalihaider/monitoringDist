...
export default function Home() {
  ...
  const [liveBlogs, setLiveBlogs] = useState<HomeBlogItem[]>([]);
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsubscribe = subscribePublishedManagedDocs(
        ...
        (nextError) => {
          setBlogsError(nextError.message);
          setBlogsLoading(false);
        },
      );
    ...
    async function loadPublicBlogs() {
      try {
        const response = await fetch("/api/docs/chapters", {
          cache: "no-store",
        });
        const result = (await response.json()) as PublicChaptersResponse;
        if (!response.ok) {
          throw new Error("Failed to load live blogs");
        }

        if (!active) return;

        const nextBlogs = (result.chapters ?? [])
          .slice(0, 6)
          .map((entry) => ({
            id: entry.id,
            slug: entry.slug,
        ...
        setLiveBlogs(nextBlogs);
        setBlogsError(null);
      } catch (nextError) {
        if (!active) return;
        const message = nextError instanceof Error ? nextError.message : "Failed to load live blogs";
  ...
  return (
    <div className="min-h-screen bg-[#f7fbfc] text-slate-900">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
        ...
        <aside
          className={`fixed inset-y-0 right-0 z-40 w-80 max-w-[84vw] border-l border-slate-200 bg-white p-5 shadow-2xl transition-transform md:hidden ${
            isMobileMenuOpen ? "translate-x-0" : "translate-x-full"
          }`}
        >
          <div className="flex items-center justify-between">
            <p className="text-sm font-bold tracking-wide text-slate-900">Navigation</p>
            <button
              onClick={() => setIsMobileMenuOpen(false)}
          ...
          <nav className="mt-5 space-y-2">
            {navItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className="block rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-100"
              >
                {item.label}
              </a>
            ))}
      ...
      <main>
        ...
        <section className="border-y border-slate-200 bg-white py-16 lg:py-20">
          <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="reveal-up reveal-delay-1 flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-700">Live Blogs</p>
                <h2 className="mt-3 text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">
                  Real-Time Updates From The Database
                </h2>
                <p className="mt-3 max-w-2xl text-sm text-slate-600">
                  New blog entries and edits appear automatically here from the managed docs collection.
                </p>
              </div>
              <Link
                href="/docs"
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {blogsLoading ? (
                Array.from({ length: 3 }).map((_, idx) => (
                  <article key={`blog-skeleton-${idx}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <article className="rounded-2xl border border-slate-200 bg-slate-50 p-5 md:col-span-2 xl:col-span-3">
                  <p className="text-sm text-slate-600">No blogs published yet.</p>
                </article>
              ) : (
                liveBlogs.map((blog) => (
                  <Link
                    key={blog.id}
                    href={`/docs/${blog.slug}`}
                    className="group rounded-2xl border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:border-slate-300"
                  >
                    ...
                    <h3 className="mt-3 text-lg font-bold text-slate-900 group-hover:text-indigo-700">{blog.title}</h3>
                    <p className="mt-2 line-clamp-3 text-sm text-slate-600">{blog.summary || "No summary provided."}</p>
                    <div className="mt-4 flex items-center justify-between text-xs font-medium text-slate-500">
                      <span>Updated: {formatBlogDate(blog.updatedAt)}</span>
                      <span className="inline-flex items-center gap-1 text-slate-700 group-hover:text-indigo-700">
                        Read
